-- =============================================================================
-- Renewals Studio — ACTIVE slot source query (LOCAL VALIDATION COPY)
-- Generated from active_dynamic.sql with the 5 bind params replaced by their
-- default literals so it runs standalone in a Snowflake worksheet (no binds).
-- Do NOT ship this copy in the app; the app uses active_dynamic.sql with binds.
-- =============================================================================
--
-- Purpose
...
WITH params AS (
    ...
    SELECT
        -- LOCAL-VALIDATION LITERALS (replaces the app's bind params so this runs
        -- standalone in a Snowflake worksheet). These are the exact defaults the
        -- app passes; the output should match the app's "today" result.
        NULL::DATE     AS as_of_date_in,   -- #1  NULL => use is_most_recent snapshot
                                           --     (to test an as-of view, set e.g. '2026-07-31'::DATE)
        1::INT         AS quarters_back,   -- #2  quarters before current
        4::INT         AS quarters_forward, -- #3 quarters after current
        75000::NUMBER  AS min_arr,         -- #4  row inclusion threshold
        100000::NUMBER AS band_cutoff      -- #5  '100k+' vs '<100k'
),
