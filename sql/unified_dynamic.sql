-- =============================================================================
-- Renewals Studio - UNIFIED source query (self-dynamic, parameterized)
-- =============================================================================
-- One rolling window (n_past + current + n_future quarters) in ONE result set,
-- replacing the separate active + historical pulls. A QUARTER_DIFF column marks
-- each row: <0 = closed/historical (BU_FC/QTD_CC carry realized churn), 0 =
-- current, >0 = future. The app splits on QUARTER_DIFF into its active (>=0)
-- and historical (<0) views client-side; CC := QTD_CC for closed quarters.
--
-- Bind variables (params CTE, qmark '?', each appears EXACTLY once):
--   ? #1 n_past      INT    default 8
--   ? #2 n_future    INT    default 4
--   ? #3 min_arr     NUMBER default 10000   (NET_ARR_USD_PRIOR_QUARTER_END >= min_arr)
--   ? #4 band_cutoff NUMBER default 100000  ('100k+' vs '<100k')
--   FastAPI: snowflake.connector.paramstyle='qmark';
--            cur.execute(open('sql/unified_dynamic.sql').read(), [8,4,10000,100000])
-- Output header matches seeds/unified.csv / app.js FIELD_KEYS (incl. EXPANSION,
-- QUARTER_DIFF, CC_REASON, NET_ARR_USD, MANAGED_BY_PARTNER_FLAG, TERRITORY_SUB_INDUSTRY_C).
-- =============================================================================

WITH params AS (
    -- The ONLY place the bind placeholders appear (qmark '?'). Everything
    -- else reads these back out of the CTE.
    SELECT
        ?::INT    AS n_past,      -- #1  8       quarters of history
        ?::INT    AS n_future,    -- #2  4       quarters forward
        ?::NUMBER AS min_arr,     -- #3  10000   row-inclusion threshold (prior-qtr-end ARR)
        ?::NUMBER AS band_cutoff  -- #4  100000  '100k+' vs '<100k' label cutoff
),
