-- =============================================================================
-- Renewals Studio — HISTORICAL slot source query (LOCAL VALIDATION COPY)
-- Generated from historical_dynamic.sql with the 3 bind params replaced by
-- their default literals so it runs standalone in a Snowflake worksheet (no
-- binds). Do NOT ship this copy in the app; the app uses historical_dynamic.sql
-- with binds.
-- =============================================================================
--
-- Purpose
...
WITH params AS (
    ...
    SELECT
        ...
        NULL::DATE     AS as_of_date_in,   -- #1  NULL => today (to test an as-of
                                           --     view, set e.g. '2026-07-31'::DATE)
        0::INT         AS quarters_back,   -- #2  0 => start of current fiscal quarter
        100000::NUMBER AS band_cutoff      -- #3  '100k+' vs '<100k'
),
asof AS (
    SELECT
